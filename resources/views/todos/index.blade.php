                ...
                <div class="card-header bg-white border-bottom py-3">
                    <div class="d-flex justify-content-between align-items-center">
                        <h4 class="mb-0 fw-bold text-dark">
                            ...
                            My Tasks <span class="badge bg-light text-dark ms-2">{{ $todos->total() }}</span>
                        </h4>
                            ...
                            <table class="table table-hover mb-0 align-middle">
                                ...
                                <tbody class="border-top">
                                    @forelse($todos as $todo)
                                        <tr class="border-bottom hover-shadow">
                                            <td class="py-3">
                                                <a href="#" class="text-decoration-none fw-semibold text-dark" 
                                                   data-bs-toggle="modal" data-bs-target="#todoModal{{ $todo->id }}"
                                                   style="max-width: 300px; display: inline-block;">
                                                    {{ Str::limit($todo->title, 45) }}
                                                </a>
                                            </td>
                                            <td class="py-3 text-center">
                                                <span class="badge fs-6 fw-bold px-2 py-1 {{ $todo->priority === 'high' ?  : ($todo->priority === 'medium' ?  : 'bg-primery') }}">
                                                    {{ $todo->priority_stars }}
                                                </span>
                                            </td>
                                            <td class="py-3 text-center">
                                                <a href="#" class="status-toggle badge fs-6 px-3 py-2 fw-semibold text-decoration-none
                                                   {{ $todo->status === 'completed' ? 'bg-success text-white shadow-sm' : 'bg-warning text-dark shadow-sm' }}"
                                                   data-todo-id="{{ $todo->id }}" style="transition: all 0.2s;">
                                                    {{ ucfirst($todo->status) }}
                                                </a>
                                            </td>
                                            <td class="py-3 text-center small text-muted">
                                                {{ $todo->updated_at?->format('M d, H:i') ?? 'Just now' }}
                                            </td>
                                            <td class="py-3 text-end">
                                                <div class="btn-group btn-group-sm" role="group">
                                                    <a href="{{ route('todos.edit', $todo) }}" class="btn btn-outline-primary border-1 shadow-sm px-3" title="Edit">
                                        ...
                                        <!-- Detail Modal -->
                                        <div class="modal fade" id="todoModal{{ $todo->id }}" tabindex="-1">
                                            <div class="modal-dialog modal-dialog-centered modal-lg">
                                                <div class="modal-content border-0 shadow-lg">
                                                    <div class="modal-header border-0 bg-light">
                                                        <h5 class="modal-title fw-bold">{{ $todo->title }}</h5>
                                                        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                                                    </div>
                                                    <div class="modal-body p-4">
                                                        <div class="row">
                                                            <div class="col-md-8">
                                                                <p class="lead mb-1">{{ $todo->description ?? 'No description added.' }}</p>
                                                            </div>
                                                            <div class="col-md-4 text-end">
                                                                <span class="badge fs-6 me-2 mb-1">{{ $todo->priority_stars }}</span>
                                                                <span class="badge fs-6 {{ $todo->status === 'completed' ? 'bg-success' : 'bg-warning' }}">
                                                                    {{ ucfirst($todo->status) }}
                                                                </span>
                                                            </div>
                                                        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    clearBtn.addEventListener('click', () => window.location.href = "{{ route('todos.index') }}");
